...
def evaluate_single_model(name: str):
    ...
    if best_metrics:
        print(f"  [训练过程最佳]")
        print(
            f"    Best AUC:         {best_metrics.get('best_auc', 'N/A'):.4f}" if 'best_auc' in best_metrics else "    Best AUC:         N/A")
        print(
            f"    Best Macro-F1:    {best_metrics.get('best_f1_macro', 'N/A'):.4f}" if 'best_f1_macro' in best_metrics else "    Best Macro-F1:    N/A")
